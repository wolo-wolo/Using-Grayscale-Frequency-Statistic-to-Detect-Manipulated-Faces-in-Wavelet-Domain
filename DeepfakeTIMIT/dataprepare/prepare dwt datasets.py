...
path = ['./extract_frames_and_faces/faces/real/',
        './extract_frames_and_faces/faces/fake_higher_quality/',
        ]
    ...
    if labels[z] == 0:
        ...
        for subdir, dirs, files in os.walk(path[z]):
            for file in files:
                ...
                if file[-4:] == format_file[z] and i < 0.6 * img_nums:
                    src = os.path.join(subdir, file)
                    dfname = src[len(path[z]): len(path[z]) + 5] + '_' + src[src.rfind('video') + 6: -8] + '_' + file
                    for f in ['cA/', 'cH/', 'cV/', 'cD/']:
                        dst = os.path.join(train_real_dir[f], dfname)
                        haar_dwt(src, dst, fre=f)

                # Take 20% as the validation set.
                elif file[-4:] == format_file[z] and i < 0.8 * img_nums:
                    src = os.path.join(subdir, file)
                    dfname = src[len(path[z]): len(path[z]) + 5] + '_' + src[src.rfind('video') + 6: -8] + '_' + file
                    for f in ['cA/', 'cH/', 'cV/', 'cD/']:
                ...
                elif file[-4:] == format_file[z] and i < img_nums:
                    src = os.path.join(subdir, file)
                    dfname = src[len(path[z]): len(path[z]) + 5] + '_' + src[src.rfind('video') + 6: -8] + '_' + file
                    for f in ['cA/', 'cH/', 'cV/', 'cD/']:
    ...
    if labels[z] == 1:
        ...
        for subdir, dirs, files in os.walk(path[z]):
            for file in files:
                ...
                if file[-4:] == format_file[z] and i < 0.6 * img_nums:
                    src = os.path.join(subdir, file)
                    dfname = src[len(path[z]): (len(path[z]) + 5)] + '_' + src[(len(path[z]) + 6): src.rfind('\\')] + \
                             '_' + file
                    for f in ['cA/', 'cH/', 'cV/', 'cD/']:
