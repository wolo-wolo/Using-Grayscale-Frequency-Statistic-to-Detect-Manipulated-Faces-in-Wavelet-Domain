# -*- coding: UTF-8 -*-
"""
All images in the original data set are subjected to Haar wavelet transform.
The obtained cA, cH, cV, and cD subband images are divided into training set, validation set, and test set respectively
"""
...
# Create training path, validation path and testing path for the four subbands after wavelet transform
...
# The original stylegan2 dataset path
path = ['../original_datasets/stargan/real/',
        '../original_datasets/stargan/fake/',
        ]
labels = [0, 1]  # 0 means true, 1 means fake
format_file = ['.png', '.png']  # corresponding image format


for z in range(len(path)):
    print('process in :', path[z])

    # Divide the datasets of real images after wavelet decomposition of four subband images
    if labels[z] == 0:
        i = 0
        for subdir, dirs, files in os.walk(path[z]):
            for file in files:

                # Take 60% as the training set.
                if file[-4:] == format_file[z] and i < 1300:
                ...
                # Take 20% as the validation set.
                elif file[-4:] == format_file[z] and i < 1600:
                    src = os.path.join(subdir, file)
                    dfname = file
                    for f in ['cA/', 'cH/', 'cV/', 'cD/']:
                ...
                # Take 20% as the test set.
                elif file[-4:] == format_file[z] and i < 2000:
                    src = os.path.join(subdir, file)
                    dfname = file
                    for f in ['cA/', 'cH/', 'cV/', 'cD/']:
    ...
    # Divide the datasets of fake images after wavelet decomposition of four subband images
    if labels[z] == 1:
        i = 0
        for subdir, dirs, files in os.walk(path[z]):
            for file in files:
