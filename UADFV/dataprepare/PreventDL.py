...
def video_record():
    for f in ['cA/', 'cH/', 'cV/', 'cD/']:
        for subdir, dirs, files in os.walk(train_real_dir[f]):
            for file in files:
                if file[:4] not in train_real_videos:
                    train_real_videos.append(file[:4])

        for subdir, dirs, files in os.walk(validation_real_dir[f]):
            for file in files:
                if file[:4] not in validation_real_videos:
                    validation_real_videos.append(file[:4])

        for subdir, dirs, files in os.walk(test_real_dir[f]):
            for file in files:
                if file[:4] not in test_real_videos:
                    test_real_videos.append(file[:4])

        for subdir, dirs, files in os.walk(train_fake_dir[f]):
            for file in files:
                if file[:4] not in train_fake_videos:
                    train_fake_videos.append(file[:4])

        for subdir, dirs, files in os.walk(validation_fake_dir[f]):
            for file in files:
                if file[:4] not in validation_fake_videos:
                    validation_fake_videos.append(file[:4])

        for subdir, dirs, files in os.walk(test_fake_dir[f]):
            for file in files:
                if file[:4] not in test_fake_videos:
                    test_fake_videos.append(file[:4])
...
for f in ['cA/', 'cH/', 'cV/', 'cD/']:
    for subdir, dirs, files in os.walk(validation_real_dir[f]):
        for file in files:
            if file[:4] in train_real_videos:
                os.remove(subdir+'/'+file)

    for subdir, dirs, files in os.walk(test_real_dir[f]):
        for file in files:
            if file[:4] in validation_real_videos:
                os.remove(subdir+'/'+file)

    for subdir, dirs, files in os.walk(validation_fake_dir[f]):
        for file in files:
            if file[:4] in train_fake_videos:
                os.remove(subdir+'/'+file)

    for subdir, dirs, files in os.walk(test_fake_dir[f]):
        for file in files:
            if file[:4] in validation_fake_videos:
                os.remove(subdir+'/'+file)
