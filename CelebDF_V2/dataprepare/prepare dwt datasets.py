...
path = ['./extract_frames_and_faces/faces/Celeb-real/',
        './extract_frames_and_faces/faces/YouTube-real/',
        './extract_frames_and_faces/faces/Celeb-synthesis/'
        ]
labels = [0, 0, 1]  # 0 means true, 1 means fake
format_file = ['.jpg', '.jpg', '.jpg']  # corresponding image format

# use official labels to spilt testing set
f = open('../original_datasets/List_of_testing_videos.txt', 'r')
content = f.readlines()
test_img = []
for name in content:
    test_img.append(name[2:].replace('\n', ''))
...
for z in range(len(path)):
    ...
    if labels[z] == 0:
        ...
        for subdir, dirs, files in os.walk(path[z]):
            for file in files:
                img = subdir.split('/')[3] + '/' + file.split('%')[0]
                if file[-4:] == format_file[z] and (img in test_img):
                    ...
                    test_num += 1
            print(subdir.split('/')[3] + ' has test_img:' + str(test_num))
        ...
        for subdir, dirs, files in os.walk(path[z]):
            for file in files:
                ...
                if file[-4:] == format_file[z] and (img not in test_img) and i < 0.8*(len(files)-test_num):
                    src = os.path.join(subdir, file)
                    dfname = file
                    for f in ['cA/', 'cH/', 'cV/', 'cD/']:
                ...
                elif file[-4:] == format_file[z] and (img not in test_img) and i < (len(files)-test_num):
                    src = os.path.join(subdir, file)
                    dfname = file
                    for f in ['cA/', 'cH/', 'cV/', 'cD/']:
